...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name("q")).sendKeys("maltipoo", Key.RETURN);
  await driver.wait(until.titleContains("maltipoo"));
  ...
  await driver.findElement(By.name("q")).sendKeys("kitten", Key.RETURN);
  // TODO Wait for the results page to load
  await driver.wait(until.titleContains("kitten"));
});
